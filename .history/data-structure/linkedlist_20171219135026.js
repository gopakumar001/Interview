    ...
    LinkedList.prototype = {
        ...
        delete: function (direction) {
            ...
            if (this._count === 1) {
                ...
                let node;
                if(direction === "head") {
                    node = this._head.getLink("right");
                    node.setLink("left", null);
                }else{
                    node = this._tail.getLink("left");
                    node.setLink("right", null);
                }
                deletedNode = this["_" + direction];
        ...
        print: function (reverse) {
            var retVal = "",
                nxtNode = reverse ? this._tail : this._head;
            while (nxtNode) {
                if (retVal != "") {
                    retVal += " --> ";
                }
                retVal += nxtNode.getValue();
                nxtNode = nxtNode.getLink(reverse ? "left" :"right");
            }
            console.log(retVal);
        }
    };
